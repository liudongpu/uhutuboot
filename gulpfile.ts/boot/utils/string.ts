export class BootUtilString {

    private static _instance = new BootUtilString();
    ...
    formatString(sString:string, oArgs:any) {
        var result = sString;
        if (oArgs != undefined) {
     ...
     * @returns 
     */
    substringAfterLast(str:string, separator:string) {
        if (this.isEmpty(str)) {
            return str;
        }
    ...
    contains(seq:string, searchSeq:string) {
     ...
     */
    replaceBetween(sInString:string, sStart:string, sEnd:string, sWith:string, fRemove:boolean) {
     ...
     * @memberOf Mexport
     */
    replaceAll(sInString:string, sReplace:string, sWith:string):string {
